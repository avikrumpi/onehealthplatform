...
API_KEY = "YOUR_API_KEY_HERE"  # Or set as environment variable: os.getenv("OPENAI_API_KEY")

# --- CORPUS FILES ---
# Path to your menstrual health corpus files
CORPUS_FILES = [
    "bengali_menstural_problem.txt",
    "bengali_menstural_problem_1.txt"
]
...
def setup_rag_system(api_key: str, model_name: str, corpus_files: list):
    ...
    llm = ChatOpenAI(
        model=model_name,
        api_key=api_key,
        temperature=0.3  # Slightly creative but factual (0.0 = deterministic, 1.0 = creative)
    )
